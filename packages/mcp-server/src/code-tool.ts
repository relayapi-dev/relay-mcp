...
const prompt = `Runs JavaScript code to interact with the Relay API.
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { RELAY_API_KEY: requireValue(readEnv('RELAY_API_KEY') ?? client.apiKey, 'set RELAY_API_KEY environment variable or provide apiKey client option'), RELAY_BASE_URL: readEnv('RELAY_BASE_URL') ?? client.baseURL ?? undefined };
