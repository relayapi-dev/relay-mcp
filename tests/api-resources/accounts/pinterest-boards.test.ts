...
describe('resource pinterestBoards', () => {
  // Mock server tests are disabled
  test.skip('retrieve', async () => {
    const responsePromise = client.accounts.pinterestBoards.retrieve('id');
  ...
  test.skip('setDefault: only required params', async () => {
    const responsePromise = client.accounts.pinterestBoards.setDefault('id', { board_id: 'board_id' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
