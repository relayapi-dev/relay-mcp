...
describe('resource pages', () => {
  ...
  test.skip('list', async () => {
    const responsePromise = client.connect.facebook.pages.list();
  ...
  test.skip('select: only required params', async () => {
    const responsePromise = client.connect.facebook.pages.select({ connect_token: 'connect_token', page_id: 'page_id' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
